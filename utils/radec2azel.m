function [A a] = radec2azel(r, d, lat, lon, time)
...
LHA = mod(ThetaLST - r,360);

%Equation 4-12 (Elevation Deg)
a = asind(sind(lat).*sind(d)+cosd(lat).*cosd(d).*cosd(LHA));

%Equation 4-13 / 4-14 (Adaptation) (Azimuth Deg)
%Az = mod(atand(-(sind(LHA).*cosd(Dec)./(cosd(lat).*sind(Dec) - sind(lat).*cosd(Dec).*cosd(LHA)))),360);
A = mod(atan2(-sind(LHA).*cosd(d)./cosd(a),...
    (sind(d)-sind(a).*sind(lat))./(cosd(a).*cosd(lat))).*(180/pi),360);
